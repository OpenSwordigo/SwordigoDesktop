...
static uint32_t __glsl_shader_vert_spv[] =
{
    ...
    0x0000002b,0x00000027,0x00000001,0x070050,0x00000007,0x0000002c,0x0000002a,0x0000002b,
...
// Draw callbacks
static void ImGui_ImplVulkan_DrawCallback_ResetRenderState(const ImDrawList*, const ImDrawCmd*)     {} // Intentionally empty
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        // Upload vertex/index data into a contiguous GPU buffer
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (desc_set != last_image_view)
                {
                    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, &desc_set, 0, nullptr);
                    last_image_view = desc_set;
                }
    ...
    // Restore scissor to full viewport
