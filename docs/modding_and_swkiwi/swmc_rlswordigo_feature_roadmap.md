...
- **Caution**: Keep the `Mini.Controls` hooks turned off. We already have a well-established and perfect touch simulating control system on PC, so this compatibility mapping should only be stubbed/inactive to prevent overriding our native mouse/keyboard touch emulation layout.
- Expose keyboard-to-touch layout structures referring to `SwKiwi`'s `controls.c` but keep active hooking off.
